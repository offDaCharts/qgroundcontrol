/****************************************************************************
** Meta object code from reading C++ file 'QGCStatusBar.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../src/ui/QGCStatusBar.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'QGCStatusBar.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_QGCStatusBar[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      22,   14,   13,   13, 0x0a,
      43,   36,   13,   13, 0x0a,
      84,   78,   13,   13, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_QGCStatusBar[] = {
    "QGCStatusBar\0\0checked\0logging(bool)\0"
    "player\0setLogPlayer(QGCMAVLinkLogPlayer*)\0"
    "event\0paintEvent(QPaintEvent*)\0"
};

void QGCStatusBar::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        QGCStatusBar *_t = static_cast<QGCStatusBar *>(_o);
        switch (_id) {
        case 0: _t->logging((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: _t->setLogPlayer((*reinterpret_cast< QGCMAVLinkLogPlayer*(*)>(_a[1]))); break;
        case 2: _t->paintEvent((*reinterpret_cast< QPaintEvent*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData QGCStatusBar::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject QGCStatusBar::staticMetaObject = {
    { &QStatusBar::staticMetaObject, qt_meta_stringdata_QGCStatusBar,
      qt_meta_data_QGCStatusBar, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &QGCStatusBar::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *QGCStatusBar::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *QGCStatusBar::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_QGCStatusBar))
        return static_cast<void*>(const_cast< QGCStatusBar*>(this));
    return QStatusBar::qt_metacast(_clname);
}

int QGCStatusBar::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QStatusBar::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
