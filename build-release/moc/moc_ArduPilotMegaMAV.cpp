/****************************************************************************
** Meta object code from reading C++ file 'ArduPilotMegaMAV.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../src/uas/ArduPilotMegaMAV.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'ArduPilotMegaMAV.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_ArduPilotMegaMAV[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      31,   18,   17,   17, 0x0a,
      80,   17,   17,   17, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_ArduPilotMegaMAV[] = {
    "ArduPilotMegaMAV\0\0link,message\0"
    "receiveMessage(LinkInterface*,mavlink_message_t)\0"
    "sendTxRequests()\0"
};

void ArduPilotMegaMAV::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        ArduPilotMegaMAV *_t = static_cast<ArduPilotMegaMAV *>(_o);
        switch (_id) {
        case 0: _t->receiveMessage((*reinterpret_cast< LinkInterface*(*)>(_a[1])),(*reinterpret_cast< mavlink_message_t(*)>(_a[2]))); break;
        case 1: _t->sendTxRequests(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData ArduPilotMegaMAV::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject ArduPilotMegaMAV::staticMetaObject = {
    { &UAS::staticMetaObject, qt_meta_stringdata_ArduPilotMegaMAV,
      qt_meta_data_ArduPilotMegaMAV, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &ArduPilotMegaMAV::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *ArduPilotMegaMAV::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *ArduPilotMegaMAV::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_ArduPilotMegaMAV))
        return static_cast<void*>(const_cast< ArduPilotMegaMAV*>(this));
    return UAS::qt_metacast(_clname);
}

int ArduPilotMegaMAV::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = UAS::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 2)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 2;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
